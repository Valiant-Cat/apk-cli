...
当任务涉及 APK、XAPK 或 AAB 的安装、名称、图标、版本号、包名、签名、环境检查或结果校验时，优先使用 `apk-cli`，不要直接拼凑零散的 `adb`、`apktool`、`bundletool`、`apksigner` 命令，除非目标就是调试底层工具链。
...
3. 只需要安装时，直接执行 `install`，让命令自动识别 APK、XAPK 或 AAB。
4. 需要修改时，对目标包先执行 `inspect`，确认当前状态和资源引用。
5. 再执行 `edit`，并显式提供 `--output`，避免产物落在不可预期的位置。
6. `edit` 完成后，再对输出文件执行一次 `inspect` 做结果校验。
...
```bash
$APK_CLI install '/abs/path/app.apk'
$APK_CLI install '/abs/path/app.xapk' --serial emulator-5554
$APK_CLI install '/abs/path/app.aab' --grant --json
```
...
- 多设备连接：`install` 必须加 `--serial <serial>`，不要让 adb 自行选择。
- 设备显示 `offline` 或 `unauthorized`：先修复设备连接或授权，再重试 `install`。
...
4. 实际执行的 `install` 或 `edit` 命令
5. 安装目标设备或输出文件路径
6. 最终安装结果或 `inspect` 校验结果
